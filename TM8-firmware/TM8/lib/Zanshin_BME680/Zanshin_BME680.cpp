...
TwoWire bmeBus(&sercom2, 4, 3); //set up second I2C bus
  ...
  */
  ...
  for (_I2CAddress = BME680_I2C_MIN_ADDRESS; _I2CAddress <= BME680_I2C_MAX_ADDRESS;
    ...
    if (i2cAddress == 0 || _I2CAddress == i2cAddress) {  // Only check if relevant
      bmeBus.beginTransmission(_I2CAddress);               // Check for BME680 here
      if (bmeBus.endTransmission() == 0) {                 // We have found a device that could be
